=============================================
CTGMediaExtractor by Geoff Clark
PUBLISHED by CLARKTRIBEGAMES LLC
VERSION ALPHA 1.0.000.0005
=============================================

SUPPORT THE DEVELOPMENT OF THIS APP @ 

 https://www.patreon.com/clarktribegames
 https://paypal.me/aznblusuazn

JOIN THE COMMUNITY ON FACEBOOK OR DISCORD

  https://facebook.com/clarktribe.games
  https://discord.gg/6kW4der

=============================================

This game only works on Microsoft Windows
(Windows 10 1809 or higher recommended) and
does require Microsoft .NET Framework 4.7.2
or higher.

http://go.microsoft.com/fwlink/?linkid=863265

=============================================

ABOUT THIS APP

This app was created to reduce the amount of
time to convert online media to a local audio
or video file - with a standalone tool.

Media created with this application is meant
for private/personal use.

Copy of this code without the content of the 
Author is prohibited.

Contact the author:  info@clarktribegames.com

=============================================

COPYRIGHT AND TRADEMARK DISCLAIMER

*ALL PRODUCT AND COMPANY NAMES ARE TRADEMARKS
OR REGISTERED(R) TRADEMARKS OF THEIR
RESPECTIVE HOLDERS.  USE OF THEM DOES NOT
IMPLY ANY AFFILIATION WITH OR ENDORSEMENT BY
THEM.*

=============================================

RELEASE NOTES

[CURRENT UPDATES]

1.0.000.0005 / 2021.11.10

- Updated Updater Method

[PREVIOUS UPDATES]

1.0.000.0004 / 2021.08.24

- Fixed issue with application crashing when pressing Go button

1.0.000.0002 / 2021.08.23

- Fixed rare issue with application freezing halfway through first conversion

1.0.000.0001 / 2021.08.23

- Fixed issue with audio file conversions not working at times
- Changed audio formats to mp3, aac, m4a, flac, wav, and opus
- Changed video formats to mp4, webm, avi, mkv, mpg, and wmv
- Added process window for user to see status as conversions occur

1.0.000.0000 / 2021.08.23

- Initial Release of this tool
- Supports YT to Audio (mp3, aac, m4a, flac, wav, vorbis, and opus formats)
- Supports YT to Video (mp4 and webm formats)
- Added YT video name extractor
- Added CTGUpdater for automated updates

=============================================